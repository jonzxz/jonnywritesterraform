...
- These modules are invoked by `nginx-playground` the `nginx-autoscaling` branch to set up a cloud environment stack consisting of VPC, 2 public subnets, 2 private subnets, the relevant route tables (and their entries), NAT, Elastic IPs, bucket for static site and the autoscaling groups.
